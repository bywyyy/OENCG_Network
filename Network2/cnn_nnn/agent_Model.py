...
dataPath = '../saveData/cnn_nnn' + timen + '.xls'
...
class CNNnet(nn.Module):
    def __init__(self, k):
        ...
        timen = time.strftime("%m%d%H%M%S")
        global dataPath
        dataPath = '../saveData/cnn_nnn' + timen + 'k' + k.__str__() + '.xls'
        w.save(dataPath)
        ...
        kernel_size = 1
        stride = 1
        setList = [in_channels, out_channels, kernel_size, stride]
        setData = "in_channels={0[0]},out_channels = {0[1]},kernel_size = {0[2]},stride = {0[3]}".format(
            setList)
