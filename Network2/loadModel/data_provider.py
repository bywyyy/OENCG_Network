...
def get_file_data(file_name):
    ...
    class MovieHandler(xml.sax.ContentHandler):
        def __init__(self):
            ...
            self.piece_data = [0] * 14
            '''数据为14位
            [0-2]:提议者id置1
            [3-5]:payoff,
            [6-8]:player是否接收提议，state0：提议不包含，state1：接受，state2：拒绝
            [9-11]:player权重,权重仅在读取每个文件的第一个数组包含
            [12]:proposal的label,0为接收，1为拒绝
            [13]:game的majority
            '''
        ...
        def startElement(self, tag, attributes):
            if tag == 'game':
                self.piece_data[13] = int(attributes['majority'])  # 读取game的majority信息
            ...
            if tag == 'response':
                ...
                if (player_agree == 'false'):
                    ...
                    self.piece_data[12] = 1  # proposal的label
