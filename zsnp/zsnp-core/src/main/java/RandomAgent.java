...
public class RandomAgent extends VotingAgent {
    ...
    /**
     * 接收玩家发送的语言信息（也会收到自己提出的语言信息），
     * 每当有玩家发送语言信息，都可以通过这个方法获取语言信息
     * 该策略中，将接收到的语言信息保存为全局变量，以便实现自己策略的时候参考使用
     * @param proposer 发送语言的玩家的编号
     * @param communicate 语言的内容
     */
    /**
     * 接收玩家发送的语言信息（也会收到自己提出的语言信息），
     *  每当有玩家发送语言信息，都可以通过这个方法获取语言信息
...
//        平台中的自由语言模块
        communicate.setCommunicateType("");
//        平台中的固定语言模块
        communicate.setCommunicateFree("");
//        平台中的表情模块
//        不高兴 1  正常 3  高兴 5
