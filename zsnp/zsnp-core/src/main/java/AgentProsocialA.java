 ...
 * @date 2018/1/19
 */
public class AgentProsocialA extends VotingAgent {
    ...
    public Map<Integer, Integer> propose() {
        ...
        for (Integer num: set){
            map.put( num, (int)Math.floor( game.getRewards(set)*(index_three_types[min_index_type][num]) ) );
            System.out.print("\r\n"+"选择的联盟以及资金分配--成员："+ num + "资金：" + (int)Math.floor( game.getRewards(set)*(index_three_types[min_index_type][num]) ));
        }
