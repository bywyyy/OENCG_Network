 ...
 * @date 2018/1/19
 */
public class AgentProsocialBConcede extends VotingAgent {
    ...
    public Map<Integer, Integer> propose() {
        ...
        if (betray_member == -1){
            signal = "Let us form a long-term grand coalition. I only ask for reasonable resources";
            signalType = 1;
        }
        else {
            signal = "Since you didn't agree the grand coalition,"+ name +","+ "I will ally with others";
            signalType = 2;
        }
        /** add signal */
        ...
        if (betray_member != -1){
            for (Coalition coa:game.getCoalitions()){
                if ((coa.getPartyNums().size() == game.getAmountOfPlayers()-1) & (!coa.getPartyNums().contains(betray_member))){
         ...
         * 3，分配总资金
         */
        float sum_set_weight = 0;
        for (Integer num:set){
            sum_set_weight += index_three_types[min_index_type][num];
        }


        for (Integer num: set){
            map.put( num, (int)Math.floor( game.getRewards(set)*(index_three_types[min_index_type][num]/sum_set_weight) ) );
            System.out.print("\r\n"+"选择的联盟以及资金分配--成员："+ num + "资金：" + (int)Math.floor( game.getRewards(set)*(index_three_types[min_index_type][num]/sum_set_weight) ));
        }
