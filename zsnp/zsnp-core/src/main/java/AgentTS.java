 ...
 */
public class AgentTS extends VotingAgent {
...
//        round += 1;
    ...
    public Map<Integer, Integer> propose() {
        ...
        if (super.roundNum < game.getMaxRoundNum() / 3) {
            ...
            if (sets_mid.size() >= 1) {
                Random random = new Random();
                int random_set_index = random.nextInt(sets_mid.size());
                set = sets_mid.get(random_set_index);
            } else {
                System.out.println("Error <<randomly choose the highest-payoff coalition set>> error!!!!");
            }
            /** c. calculate the chosen set's set_sum_weight*/
            float set_sum_weight = 0;
            for (Integer num : set) {
                set_sum_weight += index_three_types[mid_index_type][num];
            }
             ...
             */
            for (Integer num : set) {
                map.put(num, (int) Math.floor(game.getRewards(set) * (index_three_types[mid_index_type][num] / set_sum_weight)));
                System.out.print("\r\n" + "选择的联盟以及资金分配--成员：" + num + "资金：" + (int) Math.floor(game.getRewards(set) * (index_three_types[mid_index_type][num] / set_sum_weight)));
            }
        } else {
            Set<Integer> set = new HashSet<>();
            if (sets_min.size() >= 1) {
                Random random = new Random();
                int random_set_index = random.nextInt(sets_min.size());
                set = sets_min.get(random_set_index);
            } else {
                System.out.println("Error <<randomly choose the highest-payoff coalition set>> error!!!!");
            }
            /** c. calculate the chosen set's set_sum_weight*/
            float set_sum_weight = 0;
            for (Integer num : set) {
                set_sum_weight += index_three_types[min_index_type][num];
            }
             ...
             */
            for (Integer num : set) {
                map.put(num, (int) Math.floor(game.getRewards(set) * (index_three_types[min_index_type][num] / set_sum_weight)));
                System.out.print("\r\n" + "选择的联盟以及资金分配--成员：" + num + "资金：" + (int) Math.floor(game.getRewards(set) * (index_three_types[min_index_type][num] / set_sum_weight)));
            }
        }
