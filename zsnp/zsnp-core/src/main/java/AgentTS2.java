 ...
 */
public class AgentTS2 extends VotingAgent {
    ...
    public Map<Integer, Integer> propose() {
        ...
        for (Coalition coa : game.getCoalitions()) {
            if (coa.getPartyNums().contains(own.getNum())) {
                ...
                float self_reward = coa_rewards * (index_three_types[max_index_type][own.getNum()] / sum_weight_max);
                list.add(self_reward);
            }
        }
        ...
        //创建一个新的集合对象
        List selfRewardlist = new ArrayList();

        while (it.hasNext()) {
            Object obj = it.next();
            //判断新集合中是否包含此元素，如果包含就不添加
            if (!selfRewardlist.contains(obj)) {
                selfRewardlist.add(obj);
            }
        }

        Collections.sort(selfRewardlist);

        Map<Integer, Integer> map = new HashMap<>();
        float set_sum_weight = 0;
        List<Coalition> coalitionitem = new ArrayList<Coalition>();

        for (Coalition coa : game.getCoalitions()) {
            if (coa.getPartyNums().contains(own.getNum())) {
                coalitionitem.add(coa);
            }
        }
        ...
        if (super.roundNum <= game.getMaxRoundNum() / 3) {
            float self_reward = 0;
            if (selfRewardlist.size() >= 1) {
                while (self_reward != (Float) selfRewardlist.get(selfRewardlist.size() - 1)) {
                    Random random = new Random();
                    int random_set_index = random.nextInt(selfRewardlist.size());
                    set = coalitionitem.get(random_set_index).getPartyNums();
                    ...
                    int coa_rewards = game.getRewards(set);
                    self_reward = coa_rewards * (index_three_types[max_index_type][own.getNum()] / set_sum_weight);
            ...
            float self_reward = 0;
            if (selfRewardlist.size() >= 2) {
                while (self_reward != (Float) selfRewardlist.get(selfRewardlist.size() - 2)) {
                    Random random = new Random();
                    int random_set_index = random.nextInt(selfRewardlist.size());
                    set = coalitionitem.get(random_set_index).getPartyNums();
            ...
            float self_reward = 0;
            if (selfRewardlist.size() >= 3) {
                while (self_reward != (Float) selfRewardlist.get(selfRewardlist.size() - 3)) {
                    Random random = new Random();
                    int random_set_index = random.nextInt(selfRewardlist.size());
                    set = coalitionitem.get(random_set_index).getPartyNums();
