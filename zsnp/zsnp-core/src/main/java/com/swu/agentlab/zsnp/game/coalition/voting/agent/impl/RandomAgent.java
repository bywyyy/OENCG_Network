 ...
 * @date 2018/1/19
 */
public class RandomAgent extends VotingAgent {
    ...
    private int maxRoundNum;                //最多协商多少轮

    private double discount;                    //效益衰减因子
    ...
    public void init(Game game, Infos own, Map<Integer, Infos> opponents) {
        ...
        this.maxRoundNum = game.getMaxRoundNum();
    ...
    public Map<Integer, Integer> propose() {
        ...
        int rewards = 0;
        do{
            int amountOfAllies = rand.nextInt(game.getAmountOfPlayers());
            for(int i = 0; i<amountOfAllies; i++){
                int num = rand.nextInt(game.getAmountOfPlayers());
                while (num==own.getNum()){
                    num = rand.nextInt(game.getAmountOfPlayers());
                }
                set.add(num);
            }
            rewards = game.getRewards(set);
        }while (rewards == 0);
        /**
         * 3，分配总资金
         */
        ...
        for(Integer num: set){
            if(num != own.getNum()){
                int allocation = rand.nextInt(remain);
                map.put(num, allocation);
                remain = remain - allocation;
            }
        }
        map.put(own.getNum(), remain);
        /**
         *返回map对象
         */
        return map;
    ...
    public boolean response() {
        //这里示范如何获取一个经过衰减因子作用之后的净收益，
        //衰减后进收益 = 毛收益 * 衰减因子 ^ (当前回合 / 总的回合) + 开销
        int reward = lastProposal.get(own.getNum());        //毛收益
        double talent = own.getTalent();                             //开销
        double discount = this.discount;                            //衰减因子
        int passedRound = this.roundNum;                    //当前回合，这里要注意了，this.roundNum是父类的变量，父类会记录当前轮数，由于涉及多个场次，己类记录当前回合会比较麻烦，所以直接获取父类中的这个变量是最安全的一种做法
        int maxRound = game.getMaxRoundNum();           //总回合数
        double discountedUtility = this.getDiscountedUtility(reward, talent, discount, passedRound, maxRound);         //discountedUtility  = reward * Math.pow(discount, ((double)(passedRound))/maxRound) + talent;
